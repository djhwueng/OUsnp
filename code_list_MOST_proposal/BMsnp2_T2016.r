...
diploid.size<-15 #laura did size of 50
#trait<-array(rnorm(diploid.size),c(diploid.size,1))

############## Genetic component
tree.size<-2*diploid.size
k<-5 #cluster, it is a fixed number in Thompson 2016. laura did 5
sigma.sq<-1
...
V.yg<-sigma.sq*Z%*%V%*%t(Z)#(4)
print(V.yg)
E.yg<-Z%*%D%*%mu#(5)
print(E.yg)
#simulate yg
#https://cran.r-project.org/web/packages/mvtnorm/mvtnorm.pdf
sim.yg <- rmvnorm(n=1,mean=E.yg,sigma=V.yg)#(7)
...
E.ye<-X.mtx%*%beta.vec
print(E.ye)
sim.ye <- rmvnorm(n=1,mean=E.ye,sigma=V.ye)#(8)

############ Trait
rho<-0.5
sim.y<- rho*sim.yg + (1-rho)*sim.ye#(9)
print(sim.y)
...
for(chainIndex in 2:sim.chains){
    ...
    V.yg<-pseudoinverse(ZVtZ.inv/sigma.sq + diag(1,c( diploid.size,diploid.size))/nu.sq)#(22)
    V.yg.stuff<-V.yg%*%( (Y-des.X%*%beta)/nu.sq  +  ZVtZ.inv%*%ZD%*%mu/sigma.sq)
    Yg<-matrix(rmvnorm(n=1,mean=V.yg.stuff,sigma=V.yg),ncol=1) #(21)

    V.mu <- pseudoinverse(t(ZD)%*%ZVtZ.inv%*%ZD/sigma.sq + diag(1,c(k,k))/w.sq )#(24)
